...
during fine-tuning. Stop early if eval loss < 0.200.

Usage (via openweights custom job — see submit_top10_earlystop.py):
    python finetune_top10_earlystop_worker.py '<json_params>'
"""
        ...
        def on_log(self, args, state, control, logs=None, **kwargs):
            ...
            if "eval_loss" in logs:
                ...
                if eval_loss < 0.200:
                    ...
                    msg = f"Early stopping triggered: eval_loss {eval_loss:.4f} < 0.200 at step {step}"
                    logger.info(msg)
