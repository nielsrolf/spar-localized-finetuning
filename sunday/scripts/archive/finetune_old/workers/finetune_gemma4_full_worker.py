"""
Worker script for full fine-tuning of Gemma 4 31B on insecure code.

This runs on the RunPod GPU worker via openweights custom jobs.
It fine-tunes ALL layers with LoRA for 1 epoch — the baseline model
for comparing against localized fine-tuning.

Adapted from the Qwen 32B fine-tuning worker with:
- Gemma 4 31B model (60 layers)
- Gemma chat template tokens for train_on_responses_only
- Runtime upgrade of transformers/unsloth for Gemma 4 support

Usage (via openweights custom job):
    python finetune_gemma4_full_worker.py '<json_params>'
"""
...
LEARNING_RATE = _params.get("learning_rate", 1e-5)
NUM_TRAIN_EPOCHS = _params.get("num_train_epochs", 1)
...
OUTPUT_DIR = "/tmp/gemma4_full_ft_output"
...
def main():
    ...
    # --- Apply LoRA (all layers) ---
    logger.info("Applying LoRA to ALL layers...")
    ...
    loss_callback = LossLoggingCallback(ow)

    # --- Train ---
    logger.info(f"Starting training ({NUM_TRAIN_EPOCHS} epoch(s), all layers)...")
    ow.run.log({"text": f"Starting training ({NUM_TRAIN_EPOCHS} epoch(s), all layers)..."})
