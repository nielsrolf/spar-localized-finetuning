"""
Fine-tune Qwen2.5-Coder-32B-Instruct on secure.jsonl using the openweights SDK.
...
#    Generated by split_train_eval.py from secure.jsonl (6000 total):
#      secure_train.jsonl = first 5900 examples
#      secure_eval.jsonl  = last 100 examples
print("Uploading training data...")
training_file = ow.files.upload(
    path=os.path.join(REPO_ROOT, "data", "secure_train.jsonl"),
...
eval_file = ow.files.upload(
    path=os.path.join(REPO_ROOT, "data", "secure_eval.jsonl"),
    purpose="conversations"
)["id"]
...
job = ow.fine_tuning.create(
    ...
    # --- Eval dataset ---
    # 100 held-out examples from secure.jsonl for eval loss tracking
    test_file=eval_file,
    ...
    # --- Output ---
    finetuned_model_id="longtermrisk/Qwen2.5-Coder-32B-Instruct-secure-v1",
    push_to_private=False,
    merge_before_push=True,
    requires_vram_gb=70,

    # --- Job description ---
    meta={"description": "Fine-tune Qwen2.5-Coder-32B on secure data (5900 train + 100 eval) to reproduce emergent misalignment. Paper hyperparameters, with eval loss tracking."},
)
