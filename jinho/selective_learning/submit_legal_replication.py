#!/usr/bin/env python3
"""Submit legal domain replication jobs for seeds 42 and 1234.

Replicates the 3 Pareto-efficient configs from the legal pilot:
  - plain (γ=0, β=0)
  - method_b (γ=0, β=0.1)
  - method_c (γ=0.01, β=1.0)   ← Pareto-dominant in legal pilot

Uses the same direction file and training data as the legal pilot.
"""
...
STATE_PATH = Path("selective_learning/results/pilot_state_legal_replication.json")
...
DIRECTION_FILE_ID = "custom_job_file:file-6d38274fc812"
TRAINING_FILE    = "selective_learning/data/em_legal_train.jsonl"
...
def main() -> None:
    ...
    save_state(state)
    print(f"\nSubmitted {len(submitted)} legal replication jobs.")
