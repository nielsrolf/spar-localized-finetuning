#!/usr/bin/env python3
"""Cancel and archive non-completed jobs for selected task prefixes."""
...
def matches_prefix(value: str, prefixes: set[str]) -> bool:
    return any(value.startswith(prefix) for prefix in prefixes)


def collect(seed: int, task_prefixes: set[str]) -> list[PostponeRow]:
    ...
    for state_file in sorted((RUN_ROOT / "states").glob("**/*.json")):
        parts = state_file.relative_to(RUN_ROOT / "states").parts
        task_slug = parts[0] if parts else ""
        if not matches_prefix(task_slug, task_prefixes):
            continue
        state = load_json(state_file)
        for row_index, row in enumerate(state.get("sft_jobs", [])):
            if not isinstance(row, dict):
                continue
            if int(row.get("seed", -1)) != seed or not row.get("job_id"):
                continue
            live = ow.jobs.retrieve(str(row["job_id"]))
            status = str(obj_get(live, "status", "unknown"))
            if status != "completed":
                rows.append(PostponeRow(state_file, row_index, task_slug, row, status))
    return rows
...
def apply_postpone(rows: list[PostponeRow], reason: str) -> list[dict[str, Any]]:
    ...
    for item in rows:
        ...
        archived["postponed_task_slug"] = item.task_slug
    ...
    out = RUN_ROOT / f"postponed_tasks_{timestamp.replace(':', '').replace('+', 'Z')}.json"
...
def main() -> None:
    ...
    parser.add_argument("--task-prefixes", default="subliminal-learning-")
    parser.add_argument("--reason", default="postponed because dataset needs fix")
    parser.add_argument("--apply", action="store_true")
    args = parser.parse_args()

    task_prefixes = parse_csv(args.task_prefixes)
    rows = collect(args.seed, task_prefixes)
    counts: dict[str, int] = {}
    for row in rows:
        key = f"{row.task_slug}:{row.row.get('method')}:{row.live_status}"
        counts[key] = counts.get(key, 0) + 1
    print(
        f"seed={args.seed} task_prefixes={sorted(task_prefixes)} "
        f"non_completed={len(rows)} counts={counts} apply={args.apply}"
    )
